...
select first_name,last_name, job_id,salary
from employees as e
where salary = (select count(distinct salary) from employees as m
...
select concat(e.first_name,'',e.last_name) as employeename, e.salary as emp_salary,
 concat (m.first_name,'',m.last_name) as mang_name,m.salary as mang_salary
 from employees as e
 join employees as m on e.manager_id = m.employee_id;
 
 -- top 3 employee with avg salary
...
select count(*) as emp_count, year(hire_date) as hire_date
from employees
group by year(hire_date);

-- Find the salary range of employees
select max(salary)max,min(salary)min,round(avg(salary))avg
from employees;
